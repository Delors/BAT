...
object MyFirstAnalysis
  ...
  def apply(project: Project) = {
    ...
    for {classFile <- project.classFiles
      ...
      var locals = new Array[SSAVariable](method.body.get.maxLocals).map(x => new SSAVariable(VariableMaker.newVar()))
      ...
      var states = new Array[State](method.body.get.instructions.size).map(x => new State())  // Initialisieren mit null für jeden state
      ...
      while (!finished) { //Abbruch, wenn eine Schleife komplett includes korrekt war
        println("Außen while")
        finished = true
        i = 0
        while (i < method.body.get.instructions.size) {
          ...
          if (instruction != null) {
            ...
            var result: (Option[SSAInstruction], List[SSAInstruction], Array[SSAVariable], Option[List[Int]]) = null
            //Compute instruction TODO richtige stack und locals nehmen!

            // Sollte nur beim ersten Dummystate angewand werden. Danach ja übergebene und gemergede stacks.
            if(states(i).isDummy()){
              println("Initialdummy!")
              states(i) = new State(locals, stack, None)
              //val result : (Option[SSAInstruction], List[SSAInstruction], Array[SSAVariable], Option[List[Int]]) = getApplyInstruction(instruction, stack, locals, i)
              result  = getApplyInstruction(instruction, stack, locals, i)
              finished = false
            }
            else{
            ...
            val newState = new State(locals, stack, result._1)
            ...
            for(index <- indices){
              ...
              var j = index
                ...
                val nextInstructionIndex = getNextInstructionIndex(method.body.get.instructions, index)
              if(nextInstructionIndex.isDefined){
                //Auffüllen bis zur nächsten echten Instruction
                while (j < nextInstructionIndex.get+1){
                    val currentState = states(j)
                      val mergeResult = newState.merge(currentState)
                      states(j) = mergeResult._1
                      finished = finished && mergeResult._2 // Für das Abbruchkriterium notwendig; Abbruch, wenn includes jedes mal der Fall war
                    j += 1
                }
              }
            }
  ...
  def getApplyInstruction(instr: Instruction, stack: List[SSAInstruction], locals: Array[SSAVariable], index: Int):
  (Option[SSAInstruction], List[SSAInstruction], Array[SSAVariable], Option[List[Int]]) = {
    instr.opcode match {
      case 3 /*ICONST_0*/ => {
      ...
      case 161 /*IF_ICMPLT*/ => {
        ...
        val list = List.empty[Int].:+(index+offset).:+(index+1)
